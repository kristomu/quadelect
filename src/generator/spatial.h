...
// cand in order of increasing f(x,y). This is unlike, say, IIC, in that there
...
class spatial_generator : public pure_ballot_generator {
	...
	protected:
		bool uses_mean, uses_sigma;
		vector<double> mean;
		double sigma;

		virtual vector<double> rnd_vector(int size, rng &
				random_source) const;
	...
	public:
		...
		spatial_generator(bool compress_in, bool do_truncate,
			...
			pure_ballot_generator(compress_in, do_truncate) {
				...
				uses_mean = false; uses_sigma = false;
				fixed = false; }
		...
		virtual bool set_mean(const vector<double> mean_in);
		virtual bool set_sigma(double sigma_in);

		// Should also return dimensions, etc.
		virtual string name() const { return("Uniform spatial"); }
};
