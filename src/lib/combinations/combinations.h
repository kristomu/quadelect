...
rotate_discontinuous3(BidirIter first1, BidirIter last1,
	...
	if (d1 <= d2) {
		rotate_discontinuous(std::next(first2, d2 - d1), last2, d1, first3, last3,
			d3);
	} else {
		rotate_discontinuous(std::next(first1, d2), last1, d1 - d2, first3, last3,
			d3);
...
combine_discontinuous3_(BidirIter first1, BidirIter last1,
	...
	if (d != 0) {
		if (d2 > 1) {
			rotate_discontinuous3(first1, last1, d1, std::next(first2), last2, d2-1,
				first3, last3, d3);
		} else {
			rotate_discontinuous(first1, last1, d1, first3, last3, d3);
		}
	} else {
		rotate_discontinuous3(first1, last1, d1, first2, last2, d2, first3, last3,
			d3);
	}
	return false;
}
...
combine_discontinuous3(BidirIter first1, BidirIter last1,
	...
	return combine_discontinuous3_(first1, last1, d1, first2, last2, d2,
			first3, last3, d3, fbc);
}
...
count_each_permutation(BidirIter first, BidirIter mid, BidirIter last) {
	return count_each_permutation(static_cast<std::uintmax_t>(std::distance(
					first, mid)),
...
for_each_circular_permutation(BidirIter first,
	...
	for_each_combination(first, mid, last,
		detail::circular_permutation<Function &,
...
template <class BidirIter>
std::uintmax_t
count_each_circular_permutation(BidirIter first, BidirIter mid,
	BidirIter last) {
...
reversible_permutation<Function, Size>::operator()(BidirIter first,
	...
	//     the middle.
		...
		for (Size i = 1; i < s2-1; ++i, ++bm1, ++b, ++bp1) {
			...
			if (combine_discontinuous3(first, b, i, bp1, a, s2-i-1, ap1, last, s_-s2-1,
					f3)) {
				return true;
			}
		}
...
template <class BidirIter>
std::uintmax_t
count_each_reversible_permutation(BidirIter first, BidirIter mid,
	BidirIter last) {
...
class reverse_circular_permutation {
	...
	public:
		...
		operator()(BidirIter first, BidirIter last) {
			...
			return reversible_permutation<BoundFunc, D>(f, std::distance(n, last))(n,
					last);
		}
};
...
for_each_reversible_circular_permutation(BidirIter first,
	...
	BidirIter last, Function f) {
	for_each_combination(first, mid, last,
		detail::reverse_circular_permutation<Function &,
		BidirIter>(f, std::distance(first, mid)));
	return std::move(f);
}
