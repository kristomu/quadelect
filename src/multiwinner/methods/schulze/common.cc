...
void SchulzeCommon::read_ballot_input(const election_t & input,
	size_t council_size,
...
// Generate and throw an error indicating Reading_the_Input could not
// read the input file. It has to be done this way because we reference
// errno, which is a global that's set when the error occurs; we can't
// just store the exception first and *then* throw later.
void throw_fs_err() {
	throw std::system_error(errno, std::generic_category(),
		std::string("SchulzeCommon: Error reading ballot file"));
}

void SchulzeCommon::Reading_the_Input() {
	int i1,i2,i3,i4,i5;
	char c1;
	...
	// Note: the code below has been altered to use C++ exceptions
	// because g++ warns when we silently ignore errors from fscanf,
	// since that can lead to undefined behavior. -km

	while (!feof(datafile)) {
		c1=fgetc(datafile);
		switch (c1) {
			case 'M': if (fscanf(datafile,"%d\n",&M) == EOF) throw_fs_err();
				break;

			case 'C': if (fscanf(datafile,"%d\n",&C) == EOF) throw_fs_err();
				break;

			case 'N': if (fscanf(datafile,"%d\n",&N) == EOF) throw_fs_err();
				break;

			case 'F': if (fscanf(datafile,"%d\n",&i5) == EOF) throw_fs_err();
				break;
			...
			case 'B': do c1=fgetc(datafile);
				...
				switch (i5) {
					case 1: for (i1=0; i1<N; i1++) {
							...
							if (fscanf(datafile,"%d",&i3) == EOF) {
								throw_fs_err();
							}

							for (i3=0; i3<C; i3++) {
								i4=i2+i3;
								if (fscanf(datafile,"%d",&Vote[i4]) == EOF) {
									throw_fs_err();
								}
							}
...
void SchulzeCommon::Analyzing_the_Input() {
...
void SchulzeCommon::PropCompletionSimple() {
...
void SchulzeCommon::EKarpSimple() {
...
void SchulzeCommon::PropCompletion() {
	int i1,i2,i3,i4,i5,i6,i7,i8,i9,i10,i11;
	bool j1,j2,j3,j4;
	long double NewValue;
	int totValue;
