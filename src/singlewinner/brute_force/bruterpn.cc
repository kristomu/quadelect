...
bool cond_brute_rpn::check_monotonicity_single_instance(int num_attempts,
	...
	for (int i = 0; i < num_attempts; ++i) {
		...
		for (int j = 0; j < inner_tries
			&& modified_vote_array[down[which]] < 1; ++j) {
			which = random()%4;
		}
...
int cond_brute_rpn::check_weak_positional_dominance(int num_attempts)
const {
	int cur_attempt = 0;    // Don't count anything but ABCA cycles as an
	// attempt.
	int failures = 0;
