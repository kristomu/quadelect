...
template<typename T> class gen_custom_function {
	private:
		...
		mutable std::vector<T> algorithm_stack;

		size_t number_candidates;

		const double blancmange_order = 0.67;

		/* Quick and dirty way of specializing depending on T */
		T get_nan() const;
		bool is_nan(const T & in) const;
		bool is_finite(const T & in) const;
		...
		T linear_combination(const std::vector<int> & indices,
			const std::vector<T> & weights) const;

		T evaluate_ref(const atom_bundle & cur_alias,
			const std::vector<T> & input_values,
			size_t numcands) const;

		T evaluate(std::vector<T> & stack,
			const atom_bundle & cur_atom,
			const std::vector<T> & input_values, size_t numcands) const;

		T evaluate(const std::vector<atom_bundle> & algorithm,
			const std::vector<T> & input_values, size_t numcands) const;
	...
	public:
		T evaluate(const std::vector<T> & input_values) const {
...
template<class T> T gen_custom_function<T>::get_nan() const { 
	return T(std::numeric_limits<T>::quiet_NaN());
}

template<class T> bool gen_custom_function<T>::is_nan(const T & in) const {
	return (isnan(in));
}

template<typename T> bool gen_custom_function<T>::is_finite(
	const T & in) const { return finite(in); 
}

template<class T> size_t gen_custom_function<T>::get_num_referential_atoms(
	size_t numcands) const {
...
template<class T> atom_bundle gen_custom_function<T>::get_atom_bundle(
...
template<class T> std::vector<atom_bundle> 
	gen_custom_function<T>::decode_algorithm(algo_t algorithm_encoding, 
	size_t numcands) const {
...
template<class T> matrix_indices 
	gen_custom_function<T>::get_positional_matrix_indices(
	size_t numcands) const {
...
template<class T> bool gen_custom_function<T>::does_a_beat_b(int a, int b,
...
template<class T> matrix_indices 
	gen_custom_function<T>::get_pairwise_matrix_indices(
	size_t numcands) const {
...
template<class T> T 
	gen_custom_function<T>::linear_combination(
	const std::vector<int> & indices, const std::vector<T> & weights) const {
...
template<class T> T 
	gen_custom_function<T>::evaluate_ref(const atom_bundle & cur_alias,
	const std::vector<T> & input_values, size_t numcands) const {
...
template<class T> T gen_custom_function<T>::evaluate(std::vector<T> & stack,
	...
	T right_arg = *(stack.rbegin());
	if (is_nan(right_arg)) return(right_arg); // NaNs propagate automatically
	stack.pop_back();
	...
	T middle_arg = *(stack.rbegin());
	if (is_nan(middle_arg)) return(middle_arg); // NaN propagate automatically
	stack.pop_back();
	...
	if (cur_atom == ALL_FUNC_PLUS) {
		T sum_stack = std::accumulate(stack.begin(), stack.end(), T(0.0));
		stack.clear();
	...
	switch(cur_atom) {
		...
		case BINARY_FUNC_DIVIDE:
			if (!is_finite(middle_arg) && is_finite(right_arg) && right_arg != 0) {
				return(middle_arg);
			}
			// lim x->inf 3/x = 0
			if (is_finite(middle_arg) && !is_finite(right_arg)) {
				return(0);
			}
...
template<class T> T gen_custom_function<T>::evaluate(
	...
	for(const atom_bundle & atom: algorithm) {
		T output = evaluate(algorithm_stack, atom, input_values,
			numcands);

		if (is_nan(output)) return(output);
...
template<class T> std::string gen_custom_function<T>::get_atom_name(
...
template<class T> std::string gen_custom_function<T>::to_string() const {
...
template<class T> bool gen_custom_function<T>::set_algorithm(
	algo_t algorithm_encoding) {
	...
	std::vector<T> fake_input(factorial(number_candidates), 0);
	T result = evaluate(proposed_algorithm, fake_input,
		number_candidates);
...
template<class T> void gen_custom_function<T>::force_set_algorithm(
	algo_t algorithm_encoding) {
	if (current_algorithm_num == algorithm_encoding) {
		return;
	}
...
template<class T> 
bool gen_custom_function<T>::test_positional_linear_combination() {
...
template<class T>
bool gen_custom_function<T>::test_pairwise_linear_combination() {
	set_num_candidates(3);
...
template<class T> bool gen_custom_function<T>::test_pairwise_inference() {
	set_num_candidates(4);

	std::vector<T> input = {1,0,0,0,0,0,0,0,0,2,0,0,0,0,0,0,0,0,
		2,0,0,0,0,0};
...
template<class T> bool gen_custom_function<T>::test_pairwise_doubles() {
	set_num_candidates(3);

	std::vector<T> input = {1.1,0,0,0,0,0};
