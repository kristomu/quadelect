...
beats_tensor rmr1::get_k_disqualifications(const election_t & papers,
	...
	for (const std::vector<bool> & candidate_selection: hopeful_power_set) {
		...
		std::cout << "RMR1: Debug: handling set: ";
		...
		std::cout << " cardinality " << num_remaining_candidates_here << std::endl;
		...
		for (const candscore & cand_and_score: plur_outcome) {
			...
			std::cout << cand_and_score.get_candidate_num() << " scored " <<
				cand_and_score.get_score() << std::endl;
		}
		...
		for (int cand = 0; cand < num_candidates; ++cand) {
			...
			// to false. If we are above the threshold, then we don't
			// need to do anything.
			...
			for (int other_cand = 0; other_cand < num_candidates; ++other_cand) {
				...
				std::cout << "Cardinality " << num_remaining_candidates_here <<
					": Noting that " << cand << " is not disqualifying " << other_cand <<
					std::endl;
...
std::pair<ordering, bool> rmr1::elect_inner(
	...
	for (size_t cand = 0; cand < (size_t)num_candidates; ++cand) {
		...
		std::cout << "Making the case for " << cand << std::endl;

		for (int k = 2; k <= num_candidates && defeated; ++k) {
			// Check if there are any defeats against us that we
			// can erase, and check if we're undefeated in the same
			// go.
			defeated = false;
			...
			for (size_t challenger = 0; challenger < (size_t)num_candidates;
				...
				if (beats[k][cand][challenger] || !beats[k][challenger][cand]) {
					std::cout << "Found a disqualification by " << cand << " over " <<
						challenger << std::endl;
				...
				if (current_beats[challenger][cand]) {
					std::cout << "detected a beat by " << challenger << " over " << cand <<
						std::endl;
				}
			}
